...
# Six-hourly wrfout files have 4 timesteps/day; a chunk of 200 timesteps is
# ~50 days of data per dask chunk. Tune this (and the SLURM --mem for this
# script) if you see it thrashing or under-using available memory.
TIME_CHUNK = 200
...
def _preprocess_t2(ds):
    """Attach a real datetime Time coordinate and keep only T2 (as float32)."""
    times = pd.to_datetime(ds["Times"].astype(str).values, format="%Y-%m-%d_%H:%M:%S")
    ds = ds.assign_coords(Time=("Time", times))
    return ds[["T2"]].astype("float32")


def load_daily_extrema(directory, start_year, end_year):
    """
    Lazily load T2 from all wrfout_d01 files in [start_year, end_year) and
    reduce to daily max/min/mean via a dask-backed resample. Unlike the old
    implementation, no file's full data is ever materialized as a plain
    Python list across the whole decade -- xarray/dask process it chunk by
    chunk and only the (much smaller) daily-reduced arrays are held.

    Returns:
        tmax_c, tmin_c, tmean_c : dask-backed xr.DataArray
            dims (Time=day, south_north, west_east), Celsius
    """
    print("Discovering wrfout files...")
    ...
    if not files:
        raise FileNotFoundError(
            f"No wrfout files found for years [{start_year}, {end_year}) in {directory}"
        )
...
def write_extreme_stats(directory, tmax_c, tmin_c, tmean_c, start_year, end_year, scenario):
    ...
    call at the end, so the underlying T2 read is only ever materialized once,
    chunk by chunk, instead of holding the whole decade in memory up front.
    """
    ...
    ds = xr.Dataset(
        ...
        attrs={
            "description": "Six-hourly-derived extreme temperature postprocessed statistics from WRF wrfout files",
            "scenario": scenario,
    ...
    outfile = f"output_new/stats_6hextreme_temperature_{scenario}_{start_year}-{end_year}.nc"
